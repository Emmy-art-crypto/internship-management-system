...
module.exports.login_faculty = async (req, res, next) => {
  try {
    const faculty = await db.Faculty.findOne({ username: req.body.username });
    if (!faculty) {
    ...
    if (valid) {
      ...
      res.json({ id: faculty._id, username: faculty.username, token });
    } else {
...
module.exports.resetStudentPassword = async (req, res, next) => {
